...
my_results2=my_results2 %>% 
  mutate(beta2= factor(beta, levels=c(1,0.5,0.2), labels = beta_names),
...
tab_length = data.frame(beta=factor(my_results$beta_poly,levels=c("1","0.5", "0.2")),
...
tab_data = data.frame(beta=factor(my_results$beta_poly,levels=c("1","0.5","0.2")),
...
tab_data_prop = data.frame(beta=factor(my_results$beta_poly,levels=c("1","0.5", "0.2")), 
                           leeMs=my_results$length_poly_theory,
                           MC_M = my_results$length_poly_ori,
                           MC_MR = my_results$length_poly_ori_R)
...
tab_data_prop = data.frame(beta=unique(factor(my_results$beta_poly,levels=c("1","0.5", "0.2"))), 
                           bind_rows(tab_data_prop),
                           layer="Inf. length (%)")


tab_prop_DS= data.frame(beta=factor(my_results$beta_DS,levels=c("1","0.5", "0.2")), DS = my_results$LENGTH_DS) %>%
  group_by(beta) %>%
...
tab_data_final2= tab_data_final %>% 
  ...
  )) %>% select(name,starts_with('1'),starts_with('0.5'),starts_with('0.2'))
